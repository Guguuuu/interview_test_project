...
export default {
    ...
    methods: {
        onClose() {
            this.$bus.$emit('on-close2')
        },
        onConfirm() {
            const userobj = { id: this.OneData[0].id, username: this.username, userage: this.userage, sex: this.usersex, phone: this.phoneNumber, address: this.address, done: false }
            this.$bus.$emit('on-confirm2', userobj)
        }
    },
    mounted() {
        this.username = this.OneData[0].username
        this.userage = this.OneData[0].userage
        this.usersex = this.OneData[0].sex
        this.phoneNumber = this.OneData[0].phone
        this.detail = this.OneData[0].address.slice(9)
        // console.log(this.OneData[0].id);
    },
}
</script>
